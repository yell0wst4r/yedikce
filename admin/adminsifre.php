...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($new_sifre_err) && empty($confirm_sifre_err)){
        $sql = "UPDATE admin SET sifre = ? WHERE id = ?";
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> 
            <div class="form-group">
                ...
                <input type="sifre" name="new_sifre" class="form-control <?php echo (!empty($new_sifre_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $new_sifre; ?>">
                <span class="invalid-feedback"><?php echo $new_sifre_err; ?></span>
            </div>
            <div class="form-group">
                <label>Şifreyi doğrulayınız</label>
                <input type="sifre" name="confirm_sifre" class="form-control <?php echo (!empty($confirm_sifre_err)) ? 'is-invalid' : ''; ?>">
                <span class="invalid-feedback"><?php echo $confirm_sifre_err; ?></span>
            </div>
            <div class="form-group">
                <input type="submit" class="btn btn-warning active" value="Yenile">
                <a class="btn btn-secondary ml-2" href=" adminpanel.php">İptal</a>
            </div>
        </form>
    </div>    
</body>
